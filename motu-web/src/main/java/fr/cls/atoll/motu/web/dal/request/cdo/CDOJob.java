...
public class CDOJob implements Runnable {
     ...
     */
    public CDOJob(
        RequestProduct requestProduct_,
    ...
    public void run() {
        ...
        try {
            ...
            List<ExtractCriteriaLatLon> rangesToRequest = computeRangeOutOfLimit(requestProduct.getProduct(), latlon);
            ...
            for (ExtractCriteriaLatLon currentRange : rangesToRequest) {
                ...
                dalRequestManager.ncssRequest(requestProduct, ncss);
    ...
    protected void onJobStarts() {
        LOGGER.info("START CDO job, ProductId=" + requestProduct.getProduct().getProductId());
    }
    ...
    protected void onJobEnds() {
        setJobEnded(true);
        LOGGER.info("END CDO job, ProductId=" + requestProduct.getProduct().getProductId());
    }
