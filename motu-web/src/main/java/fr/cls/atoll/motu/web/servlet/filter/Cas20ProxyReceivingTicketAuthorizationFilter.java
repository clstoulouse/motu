...
public class Cas20ProxyReceivingTicketAuthorizationFilter implements Filter {
    ...
    /**
     * match_ldap_vs_motu (Find the service in (MotuConfig, list of ConfigService) and check profiles from
     * ldap attributes vs profile from MotuConfig)
     * 
    ...
    private boolean match_ldap_vs_motu(Map attributes, MotuConfig conf, String selected_service) {
        ...
        for (; attributeNames.hasNext();) {
            ...
            if (attr_spl.length != 5)
                continue; // check valid attribute [name,value]
        ...
        if (profiles_ldap == null || profiles_ldap.isEmpty())
            return false;
        if (status_ldap == null || status_ldap.isEmpty())
            return false;

        // Check status (must be active)
        if (!status_ldap.equals("active"))
            return false;
        ...
        if (profiles_conf != null && !profiles_conf.isEmpty()) {
            ...
            for (String p : profiles_c) {
                for (String l : profiles_l) {
                    if (p.trim().equals(l.trim()))
                        return true; // profile match
                }
            }
