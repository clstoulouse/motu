 ...
 * (C) Copyright 2009-2010, by CLS (Collecte Localisation Satellites)
 * 
 * 01/2016 : Hack because of package visibility of java.net.InMemoryCookieStore.<br/>
 * There is certainly a better way to go, but it is just fixed before a full refactoring of the project.
 * 
 */
public class PublicInMemoryCookieStore implements CookieStore {

    public PublicInMemoryCookieStore() {
    ...
    /**
     * Get all cookies, which: 1) given uri domain-matches with, or, associated with given uri when added to
     * the cookie store. 3) not expired. See RFC 2965 sec. 3.3.4 for more detail.
     */
    @Override
    ...
    /*
     * This is almost the same as HttpCookie.domainMatches except for one difference: It won't reject cookies
     * when the 'H' part of the domain contains a dot ('.'). I.E.: RFC 2965 section 3.3.2 says that if host is
     * x.y.domain.com and the cookie domain is .domain.com, then it should be rejected. However that's not how
     * the real world works. Browsers don't reject and some sites, like yahoo.com do actually expect these
     * cookies to be passed along. And should be used for 'old' style cookies (aka Netscape type of cookies)
     */
